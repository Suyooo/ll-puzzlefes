...
<div class="relative flex flex-col w-full items-center" in:fade={{duration: 100}} tabindex="-1">
    ...
    <div class="w-full mt-8 max-w-3xl flex-grow flex flex-wrap content-start gap-y-2 sm:gap-y-0">
        ...
        <MemberButton color="#B1F69C" {flip} flipClue="·5··" name="Emma" on:click={modal("Emma's Puzzle", PuzzleEmma)}/>
        <MemberButton color="#D0CEE1" {flip} flipClue="···1··" name="Rina"
                      on:click={modal("Rina's Puzzle", PuzzleRina)}/>
        <MemberButton color="#24BD8B" {flip} flipClue="···7" name="Shioriko"
                      on:click={modal("Shioriko's Puzzle", PuzzleShioriko)}/>
        <MemberButton color="#F1F0E6" {flip} flipClue="·····2" name="Mia" on:click={modal("Mia's Puzzle", PuzzleMia)}/>
        <MemberButton color="#F8C8C4" {flip} flipClue="·3··" name="Lanzhu"
                      on:click={modal("Lanzhu's Puzzle", PuzzleLanzhu)}/>
        <MemberButton color="#000" {flip} flipClue="·9·" name="Yu" on:click={modal("Yu's Puzzle", PuzzleYu)} whiteText/>
        <div class="w-full h-8">&nbsp;</div>
        <MemberButton color="#FF7F27" {flip} flipClue="7·····" name="Kanon"
                      on:click={modal("Kanon's Puzzle", PuzzleKanon)}/>
        <MemberButton color="#A0FFF9" {flip} flipClue="·····9·" name="Keke"
                      on:click={modal("Keke's Puzzle", PuzzleKeke)}/>
        <MemberButton color="#FF6E90" {flip} flipClue="··5·" name="Chisato"
                      on:click={modal("Chisato's Puzzle", PuzzleChisato)}/>
        <MemberButton color="#74F466" {flip} flipClue="······8" name="Sumire"
                      on:click={modal("Sumire's Puzzle", PuzzleSumire)}/>
        <MemberButton color="#0000A0" {flip} flipClue="···3" name="Ren" on:click={modal("Ren's Puzzle", PuzzleRen)}
                      whiteText/>
        <MemberButton color="#FFF442" {flip} flipClue="·····2" name="Kinako"
                      on:click={modal("Kinako's Puzzle", PuzzleKinako)}/>
        <MemberButton color="#FF3535" {flip} flipClue="4····" name="Mei" on:click={modal("Mei's Puzzle", PuzzleMei)}/>
        <MemberButton color="#B2FFDD" {flip} flipClue="6···" name="Shiki"
                      on:click={modal("Shiki's Puzzle", PuzzleShiki)}/>
        ...
        <div class="w-full my-6 px-2 w-full font-bold flex gap-x-4">
            ...
            {:else if solved === 40}
                <button class="w-full flex flex-col sm:flex-row items-center justify-center rounded-full p-1 uppercase select-none outline outline-[.125rem] outline-offset-[-.125rem] bg-primary outline-primary hover:bg-primary-300"
            ...
            {:else}
                <button class="basis-2/3 flex flex-col sm:flex-row items-center justify-center rounded-full p-1 uppercase select-none outline outline-[.125rem] outline-offset-[-.125rem] bg-primary outline-primary hover:bg-primary-300 flex-shrink-0"
                ...
                <button class="basis-1/3 flex flex-col sm:flex-row items-center justify-center rounded-full p-1 uppercase select-none outline outline-[.125rem] outline-offset-[-.125rem] bg-gray-300 outline-gray-300 hover:bg-gray-200 flex-shrink-1 text-sm"
                        on:click={() => flip = !flip}>
                    Toggle Clues
                </button>
            {/if}
